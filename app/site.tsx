...
function Trust() { return <div className="trust"><b>★ 4.7/5</b><span>~5,000 verified public reviews</span><span><strong><a href="https://share.google/ndN62j4GzdFXtM4Ks">Google (1,551)</a></strong> & <strong><a href="https://jsdl.in/DT-28ERCCXN">JustDial (3,409)</a></strong></span></div> }
...
function Contact(){
 ...
 const submit = async (e:FormEvent<HTMLFormElement>) => {
  ...
  setSubmitting(true);
  try{
   const res = await fetch("/api/submit-lead",{ method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ token, name:fd.get("name"), phone:fd.get("phone"), course:fd.get("course") }) });
   const data = await res.json();
   if(!data.success){ setCaptchaError(data.error==="captcha"?"Captcha verification failed. Please try again.":"Something went wrong. Please try again or call us."); recaptchaRef.current?.reset(); setSubmitting(false); return; }
  }catch{ setCaptchaError("Could not submit. Please try again or call us."); recaptchaRef.current?.reset(); setSubmitting(false); return; }
  router.push(`/thank-you?course=${encodeURIComponent(String(fd.get("course")||""))}`);
 };
...
export default function SitePage({page}:{page:string}){
 ...
 else if(page==="thank-you") content=<ThankYou/>;
 else content=<Home/>;
